let INSTALLED_BANK=window.QuestionBankAPI?.getInstalled();if(INSTALLED_BANK?.manifest?.version===window.PDF_BANK?.version&&Number(INSTALLED_BANK.manifest.answerRevision||0)<Number(window.PDF_BANK.answerRevision||0)){window.QuestionBankAPI?.restoreBuiltin();INSTALLED_BANK=null}const BANK=INSTALLED_BANK?.manifest||window.PDF_BANK,CARDS=INSTALLED_BANK?.cards||window.CARD_BANK,KEY=`dataAnalysisQuiz.${BANK.version}`,state=JSON.parse(localStorage.getItem(KEY)||'{"days":{},"wrong":[]}'),$=s=>document.querySelector(s),letters='ABCD',dayKey=new Intl.DateTimeFormat('sv-SE',{timeZone:'Asia/Shanghai'}).format(new Date()),FIRST_DAY='2026-09-09';let mode='today',historyDate=null,calendarCursor=new Date(new Date().getFullYear(),new Date().getMonth(),1);
const SCHEDULE_APPEND=Array.isArray(BANK.scheduleAppend)?BANK.scheduleAppend:[],SCHEDULE_APPEND_SET=new Set(SCHEDULE_APPEND),FIXED_GROUP_ORDER=[...shuffle(BANK.groups.map(g=>g.id).filter(id=>!SCHEDULE_APPEND_SET.has(id)),`${BANK.version}:fixed-schedule-v1`),...SCHEDULE_APPEND];
...
if(BANK.answerRevision&&state.answerRevision!==BANK.answerRevision){const rebuilt=[];for(const day of Object.values(state.days||{}))for(const id of day.submitted||[]){const group=getGroup(id),chosen=day.answers?.[id]||[];if(group)group.answers.forEach((answer,index)=>{if(chosen[index]&&chosen[index]!==answer)rebuilt.push({groupId:id,index})})}state.wrong=rebuilt;state.answerRevision=BANK.answerRevision;save()}
...
function cropGroupCard(g,n,d=null,review=false){
  ...
  if(mistakeReview){
    const panel=document.createElement('section');panel.className='material-review-notes';
    [...wrongIndexes].sort((a,b)=>a-b).forEach(i=>{const key=`${g.id}:${i}`,questionNo=card.questions?.[i]?.label||i+1,item=document.createElement('div'),note=document.createElement('textarea'),noteId=`note-${g.id}-${i}`;item.className='material-note-item';const crops=card.questionCrops?.[i]||[];if(crops.length){const cropStack=document.createElement('div');cropStack.className='wrong-question-crops';crops.forEach((src,cropIndex)=>{const img=document.createElement('img');img.src=src;img.loading='lazy';img.alt=`${g.title} 第 ${questionNo} 题原题裁剪 ${cropIndex+1}`;cropStack.appendChild(img)});item.appendChild(cropStack)}item.appendChild(rows[i]);note.id=noteId;note.setAttribute('aria-label',`第 ${questionNo} 题错题解析`);note.maxLength=5000;note.rows=4;note.placeholder=`写下第 ${questionNo} 题的错误原因、关键公式、简算方法或注意事项…`;note.value=state.notes[key]||'';note.oninput=()=>{state.notes[key]=note.value;save()};item.appendChild(note);panel.appendChild(item)});
    c.querySelector('.answer-sheet').remove();c.querySelector('.material-content').appendChild(panel);
  }
